...
export const apiBuilder = (config: {
  ...
  const processAccepts = (
    endpoint: EndpointBase,
    annotationsRequestBody?: any,
    annotationsParams?: any,
  ) => {
    const result = {} as any;
  ...
  const buildParams = (
    ...
    const pathParams = getParamsFromPath(endpoint.path) as Record<
      string,
      boolean
    >;
    ...
    for (const name in paramSchema.shape) {
      ...
      if (inType === "path" && pathParams[name] === undefined) {
        addErrorMessage(
          endpoint,
          RouteManagerErrors.PathMissingParameter(endpoint.path, name),
          "error",
        );
      ...
      parameters.push(
        schemaProcessor.processParameter(item, paramSchema.shape[name]),
      );
    }
    ...
    if (inType == "path" && falseRemainder.length) {
      for (const name of falseRemainder) {
        addErrorMessage(
          endpoint,
          RouteManagerErrors.ParameterInPathNotDeclared(endpoint.path, name),
          "error",
        );
        if (config.failOnError) {
          throwLastError();
  ...
  const processReturns = (endpoint: EndpointBase, annotations?: any) => {
    ...
    for (const statusCode in returns) {
      ...
      if (!annotation.description) {
        addErrorMessage(
          endpoint,
          RouteManagerErrors.ResponseDescriptionMissing(statusCode),
          "error",
        );
        if (config.failOnError) {
          throwLastError();
  ...
  const buildEndpointBody = (endpoint: EndpointBase, allAnnotations?: any) => {
    const annotations =
      allAnnotations?.operations?.[endpoint.operationId] ?? {};
    const accepts = processAccepts(
      endpoint,
