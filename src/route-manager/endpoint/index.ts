...
export type StatusCodeRecord = Partial<
  Record<ValidStatusCodes, z.ZodType<any>>
>;
...
export type InferRequestAccepts<
  Accepts extends EndpointBase["accepts"],
  AcceptKey extends "path" | "query" | "body",
> = AcceptKey extends keyof Accepts
  ? Accepts[AcceptKey] extends z.ZodObject<any>
    ? z.infer<Accepts[AcceptKey]>
    : never
  : never;
export type InferRequest<Endpoint extends EndpointBase> =
...
export type InferResponses<Endpoint extends EndpointBase> =
  Endpoint["returns"] extends StatusCodeRecord
    ? {
        [Status in keyof Endpoint["returns"] as Status]: Endpoint["returns"][Status] extends z.ZodType<any>
          ? z.infer<Endpoint["returns"][Status]>
          : never;
      } & {
        defaultSuccess: Endpoint["returns"][200] extends z.ZodType<any>
          ? z.infer<Endpoint["returns"][200]>
          : never;
      }
    : never;

export type EndpointArrayByOperationIds<Endpoints extends EndpointBase[]> = {
  [Item in Endpoints[number] as Item["operationId"]]: Item;
};

export type MakeOperationGroup<Endpoints extends EndpointBase[]> =
  EndpointArrayByOperationIds<Endpoints>;

export type OperationGroup = EndpointArrayByOperationIds<EndpointBase[]>;
